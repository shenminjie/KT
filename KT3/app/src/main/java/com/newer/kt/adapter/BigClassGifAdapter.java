...
public class BigClassGifAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private BigClassRoom bigClassRoom;
    private List<Action> mList;
    private MyLinearLayoutManager linearLayoutManager;
    public BigClassGifAdapter(List<Action> mList, Context mContext, MyLinearLayoutManager linearLayoutManager) {
        this.mList = mList;
    ...
    public void refresh(BigClassRoom bigClassRoom){
        this.bigClassRoom =  bigClassRoom;
        this.mList.clear();
        this.mList.addAll(bigClassRoom.getActions());
        this.notifyDataSetChanged();
    }

    @Override
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if(mList.size()>0  && mList.get(position).getPicture().length()>0) {
...
//                .setFailureDrawableId(R.mipmap.loadfailed)
                    ...
                    .build();
            x.image().bind(((ContentViewhoder) holder).mImage, mList.get(position).getPicture(), imageOptions);
        }
        if (getItemViewType(position) == IS_TITLE) {
            ((BigClassGifAdapter.HeadHolder) holder).tv_title.setText(mList.get(position).getName());
        } else {
            ((BigClassGifAdapter.ContentViewhoder) holder).mContent.setText(mList.get(position).getName());
            GifDrawable gifFromResource = null;
            try {
                if(new File(mContext.getCacheDir()+"/"+bigClassRoom.getName()+"/"+mList.get(position).getName()+".gif").exists()){
                    gifFromResource = new GifDrawable(mContext.getCacheDir()+"/"+bigClassRoom.getName()+"/"+mList.get(position).getName()+".gif");
...
//                                }
    ...
    public int getItemViewType(int position) {
        if (mList.get(position).getName().contains(" ")) {
            return IS_TITLE;
        } else {
            return IS_CONTENT;
        }
