    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        QueryBuilder.build("offline/get_school_course_data_classes").add("club_id", clubid).get(new QueryBuilder.EnhancedCallback("classes") {
            ...
            public void onSuccessWithObject(String namelink, final Object object) {
                lt = object;
                ActivityChooseClass.maps = (List<Map<String, Object>>) lt;
                ActivityChooseClass.initData();
                QueryBuilder.build("school_class/get_user_info").add("user_id", getUserid()).get(new QueryBuilder.EnhancedCallback() {
                    @Override
...
//Toast.makeText(Student_Info.this,object.toString(),Toast.LENGTH_SHORT).show();
                        ...
                        ((TextView) findViewById(R.id.tv_stuInfo4)).setText(ActivityChooseClass.get(map.get("cls").toString()));
...
//            findViewById(R.id.finish).setVisibility(View.);
            findViewById(R.id.saishi).setVisibility(View.VISIBLE);
        }
        findViewById(R.id.finish).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                final Map m = getStu(getUserid(), clubid);
        ...
        findViewById(R.id.saishi).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                final Map m = getStu(passid, clubid);
                QueryBuilder.build("school_class/update_user_info")
...
//
                        ...
                        .add("school_class_id", "")
                        .add("club_id", clubid)
                        .add("user_id", getUserid()).post(new QueryBuilder.EnhancedCallback("response;msg") {
                    @Override
                    public void onSuccessWithObject(String namelink, Object object) {
                        if (namelink.equals("response")&&object.toString().equals("success")) {
                            setResult(2,new Intent().putExtra("map", (Serializable) m).putExtra("removed","1"));
                            finish();
                        }else{
//                            Toast.makeText("");
                        }
